...
export function registerPeerSendTool(pi: ExtensionHostLike, deps: PeerSendDeps): void {
  pi.registerTool({
    ...
    label: 'Peer Send',
    description:
      'Send a message to another live peer instance by name (see `/peers`). It is delivered as a real prompt: it steers the peer mid-turn or wakes it if idle. Fire-and-forget — the peer\'s reply arrives as a separate peer message.',
    parameters: {
      type: 'object',
      properties: {
