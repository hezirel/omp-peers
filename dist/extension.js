...
import { bridgeResolvesHost, claimBridgedPeer, listLocalAgentIds, probeHost, releaseBridgedPeer, } from './peers/host.js';
...
async function tick(st) {
    ...
    if (derived.rejected !== undefined && derived.rejected !== st.lastRejectedSessionName) {
        ...
        warnOf(st, `session name "${derived.rejected}" can't be a peer name (1-24 of a-z A-Z 0-9 _ . -); ` +
...
function syncBridge(st, others) {
    ...
    for (const record of others) {
        ...
        if (!st.claimed.has(record.name)) {
            const ok = claimBridgedPeer(BRIDGE, record, st.name, () => (st.inboundHop === undefined ? 0 : st.inboundHop + 1), (socket, frame) => requestPeer(socket, frame), (text) => logOf(st, text));
            if (ok)
