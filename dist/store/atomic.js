...
import { copyFile, mkdir, open, readFile, readdir, rmdir, stat, unlink, } from 'node:fs/promises';
 ...
 */
export async function mkdirLock(dir, opts = {}) {
    ...
    for (;;) {
        ...
        if (Date.now() >= deadline) {
            throw new LockError(`timed out after ${timeoutMs}ms waiting for lock ${dir}` +
...
export async function unlockDir(dir) {
    try {
        await rmdir(dir);
    }
    catch {
 ...
 */
export async function withRegistryLock(dir, fn, opts = {}) {
